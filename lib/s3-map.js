...
var copy_data_nmisstatic =  function(original_key, bucket) {
    ...
    if (match) {
        ...
        var dest_options = {bucket: 'formhub-ossap'};
        var dest_key = '/ossap/attachments/' + dest_photo;
        console.log('from:', original_key)
        console.log('to:', dest_key)
        bucket.copyTo(original_key, dest_options, dest_key, function(err, ret) {
            if (err) {
                console.log(err);
            }
        }).end();
...
module.exports = {
    ...
    upload: upload,
    copy: copy,
    copy_data_nmisstatic: copy_data_nmisstatic
};
