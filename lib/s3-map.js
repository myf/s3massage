...
var list_map = function(bucket_name, options, cb) {
    ...
    var counter = 0;
    var tryCounts = options.tryCounts;
...
var download = function(k, b, t, d) {
    do_with_retry(b.getFile(k), t, d, function(err,res) {
...
var del = function(k, b, t, d) {
    do_with_retry(b.del(k).on('response', function(res) {
            console.log("DELETE:", res.statusCode, k);
        }).end(), t, d, function(err){
            if (err) logfile.write('[ERROR] ' + err + '\n');
    })
};
...
var copy = function(bucket_from, bucket_to, options) {
    ...
    return list_map(bucket_from, options, function(k, b, t, d) {
        console.log("copying", k, "to", bucket_to)
        do_with_retry(b.copyTo(k, dest_options, k, function(err, ret) {
            if (err) {
                console.log(err);
            } else {
