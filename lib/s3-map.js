...
var list_map = function(bucket_name, options, cb) {
    ...
    lister
        ...
        .on('end', function () {
            logfile.write('done.. there were %s matching results', counter);
    });
};
...
var upload = function(bucket_name, dir) {
    ...
    walker.on('file', function(root, fileStats, next) {
        ...
        logfile.write("begin reading ", file_name);
        bucket.putFile(file_name, file_name, function(err, res) {
            logfile.write("finish reading ", file_name);
        });
        next();
    });
        ...
        if (res) {
            ...
            fs.exists(path.dirname(full_path), function(e) {
                if (e) {
                    ...
                    logfile.write("writing in ", full_path);
                    res.pipe(ws).on('finish', function() {
                        logfile.write(" finished writing ", full_path);
                    });
                } else {
...
var del = function(k, b) {
    b.del(k).on('response', function(res) {
            logfile.write("DELETE:", res.statusCode, k);
        }).end();
};
...
var copy = function(bucket_from, bucket_to, options) {
    ...
    return list_map(bucket_from, options, function(k, b) {
        logfile.write("copying", k, "to", bucket_to)
        do_with_retry(b.copyTo(k, dest_options,k).end());
    });
};


var do_with_retry = function(fn, tryCount, delay) {
    var tryCount = tryCount || 5;
    ...
    try {
        ...
        if (tryCount == 0) {
            ...
            logfile.write("retry:", error);
            setTimeout(do_wit_retry(fn, tryCount), delay);
        }
    }
}
