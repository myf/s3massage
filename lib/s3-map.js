...
var log = function(args) {
    var logline =  Array.prototype.slice.call(arguments).join(' ') + '\n';
...
var list_map = function(bucket_name, options, cb) {
    ...
    lister
        ...
        .on('end', function () {
            log('done.. there were %s matching results', counter);
    });
};
...
var upload = function(bucket_name, dir) {
    ...
    walker.on('file', function(root, fileStats, next) {
        ...
        log("begin reading ", file_name);
        bucket.putFile(file_name, file_name, function(err, res) {
            log("finish reading ", file_name);
        });
        next();
    });
        ...
        if (res) {
            ...
            fs.exists(path.dirname(full_path), function(e) {
                if (e) {
                    ...
                    log("writing in ", full_path);
                    res.pipe(ws).on('finish', function() {
                        log(" finished writing ", full_path);
                    });
                } else {
